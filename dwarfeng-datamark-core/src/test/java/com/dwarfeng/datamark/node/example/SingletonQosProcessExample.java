...
public class SingletonQosProcessExample {
    ...
    static void main() {
        ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext(
                "classpath:com/dwarfeng/datamark/node/spring/singleton/application-context*.xml"
        );
        ...
        try {
            ...
            CT.trace("处理器 " + handlerName + " 当前的数据标记值为: " + currentValue);
        } catch (Exception e) {
            LOGGER.warn("获取当前的数据标记值失败, 异常信息如下: ", e);
        }
        System.out.print("请按回车键继续...");
        ...
        try {
            ...
            CT.trace("处理器 " + handlerName + " 当前的数据标记值为: " + currentValue);
            CT.trace("请查看 ${datamark.resource.url} 对应的资源中的内容，观察是否与您指定的新的数据标记值一致");
        } catch (Exception e) {
